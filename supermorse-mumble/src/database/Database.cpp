...
#include "MariaDBConnectionParameter.h"
#include "MetaTable.h"
...
namespace db {
	...
	void Database::connectToDB(const ConnectionParameter &parameter) {
		...
		switch (m_backend) {
			...
			case Backend::MariaDB: {
#ifdef MUMBLE_DISABLE_MYSQL
				throw InitException("Tried to connect to MariaDB database, but MySQL support is disabled");
#else
				static bool registered      = false;
				if (!registered) {
					register_factory_mysql();
				...
				const auto &mariadbParameter = static_cast< const MariaDBConnectionParameter & >(parameter);

				connectionString = "mysql://dbname='" + mariadbParameter.dbName + "'";

				if (!mariadbParameter.userName.empty()) {
					connectionString += " user='" + mariadbParameter.userName + "'";
				}
				if (!mariadbParameter.password.empty()) {
					connectionString += " password='" + mariadbParameter.password + "'";
				}
				if (!mariadbParameter.host.empty()) {
					connectionString += " host='" + mariadbParameter.host + "'";
				}
				if (!mariadbParameter.port.empty()) {
					connectionString += " port='" + mariadbParameter.port + "'";
				}
#endif
				break;
			}
